...
private final class FloatingDashboardView: NSView {
    ...
    private func rebuildContent() {
        ...
        text("企微消息助手", 24, top - 54, 240, size: 22, weight: .semibold)
        text("MAC", 340, top - 50, 56, size: 12, weight: .semibold, color: .tertiaryLabelColor)
        CATransaction.begin()
...
private final class AgentApp: NSObject, NSApplicationDelegate {
    ...
    override init() {
        ...
        let bundledRoot = Bundle.main.resourceURL?.appendingPathComponent("wecom-gui").path
        root = ProcessInfo.processInfo.environment["WECOM_GUI_ROOT"] ??
            (bundledRoot.flatMap { FileManager.default.fileExists(atPath: $0) ? $0 : nil }) ??
            resourceRoot ?? app.deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent().path
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        let applicationMenu = NSMenu(title: "WeCom Capture")
        for (title, action, key) in [("设备配置…", #selector(configureDevice), ","),
                                     ("检查系统权限", #selector(checkPermissions), ""),
                                     ("显示/隐藏控制面板", #selector(toggleDashboard), ""),
                                     ("退出 WeCom Capture", #selector(quit), "q")] {
            let entry = NSMenuItem(title: title, action: action, keyEquivalent: key)
            entry.target = self; applicationMenu.addItem(entry)
        }
        ...
        add("设备配置", #selector(configureDevice))
        menu.addItem(.separator())
        add("显示/隐藏状态浮窗", #selector(toggleDashboard)); add("查看本机流转详情", #selector(showDetails)); add("打开运行日志", #selector(openLogs))
        menu.addItem(.separator()); add("退出 WeCom Capture", #selector(quit))
        item?.menu = menu
        render()
        refresh()
#if !DESKTOP_RENDER_TEST
        timer = Timer.scheduledTimer(withTimeInterval: 2, repeats: true) { [weak self] _ in self?.refresh() }
        if isStandalone && !FileManager.default.fileExists(atPath: configURL.path) {
            DispatchQueue.main.async { [weak self] in self?.configureDevice() }
        }
#endif
    }

    private var isStandalone: Bool {
        FileManager.default.isExecutableFile(atPath: "\(root)/../python/bin/python3")
    }
    private var supportURL: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Application Support/WeCom Capture", isDirectory: true)
    }
    private var configURL: URL { supportURL.appendingPathComponent("device.json") }
    private var childEnvironment: [String: String] {
        var env = ProcessInfo.processInfo.environment
        guard isStandalone else { return env }
        for key in Array(env.keys) where key.hasPrefix("WECOM_") || key.hasPrefix("PYTHON") {
            env.removeValue(forKey: key)
        }
        env["PATH"] = "/usr/bin:/bin:/usr/sbin:/sbin"
        env["WECOM_GUI_PYTHON"] = "\(root)/../python/bin/python3"
        env["WECOM_GUI_PYTHONPATH"] = "\(root):\(root)/../site-packages"
        env["WECOM_DESKTOP_CONFIG"] = configURL.path
        env["WECOM_CHANNEL_ENV"] = "/dev/null"
        env["WECOM_GUI_STATE_DIR"] = supportURL.appendingPathComponent("state").path
        env["WECOM_GUI_RUN_DIR"] = supportURL.appendingPathComponent("logs").path
        env["WECOM_GUI_CAPTURE_IMAGE_DIR"] = supportURL.appendingPathComponent("images").path
        env["WECOM_GUI_AX_HELPER"] = "\(root)/../helpers/ax_wecom"
        env["WECOM_GUI_OCR_HELPER"] = "\(root)/../helpers/vision_ocr"
    ...
    @objc private func configureDevice() {
        guard !actionInFlight else { return }
        let existing = (try? Data(contentsOf: configURL)).flatMap {
            try? JSONDecoder().decode([String: String].self, from: $0)
        } ?? [:]
        let alert = NSAlert()
        alert.messageText = "连接中台"
        alert.informativeText = "填写管理员分配的设备信息。修改配置前请暂停接收。"
        alert.addButton(withTitle: "保存")
        alert.addButton(withTitle: "取消")
        let fields: [(String, String, NSTextField)] = [
            ("中台地址", "WECOM_CHANNEL_BASE_URL", NSTextField()),
            ("设备 ID", "WECOM_CHANNEL_DEVICE_ID", NSTextField()),
            ("设备 Token", "WECOM_CHANNEL_DEVICE_TOKEN", NSSecureTextField())
        ]
        let view = NSView(frame: NSRect(x: 0, y: 0, width: 400, height: 150))
        for (index, entry) in fields.enumerated() {
            let y = CGFloat(110 - index * 50)
            let label = NSTextField(labelWithString: entry.0)
            label.frame = NSRect(x: 0, y: y + 24, width: 400, height: 20)
            entry.2.frame = NSRect(x: 0, y: y, width: 400, height: 24)
            entry.2.stringValue = existing[entry.1] ?? ""
            view.addSubview(label); view.addSubview(entry.2)
        }
        alert.accessoryView = view
        NSApp.activate(ignoringOtherApps: true)
        guard alert.runModal() == .alertFirstButtonReturn else { return }
        let values = Dictionary(uniqueKeysWithValues: fields.map {
            ($0.1, $0.2.stringValue.trimmingCharacters(in: .whitespacesAndNewlines))
        })
        guard let url = URLComponents(string: values["WECOM_CHANNEL_BASE_URL"] ?? ""),
              url.scheme == "https", url.host?.isEmpty == false, url.user == nil, url.password == nil,
              url.query == nil, url.fragment == nil,
              values.values.allSatisfy({ !$0.isEmpty }) else {
            let error = NSAlert(); error.messageText = "请填写有效的 HTTPS 地址、设备 ID 和 Token"
            error.runModal(); return
        }
        // Changing credentials while a worker holds an old device identity is unsafe.
        let running = snapshot.states.contains { $0.process == "edge_channel" && !["stopped", "idle"].contains($0.status) }
        guard !running else {
            let error = NSAlert(); error.messageText = "请先暂停接收，再修改设备配置"
            error.runModal(); return
        }
        do {
            try FileManager.default.createDirectory(at: supportURL, withIntermediateDirectories: true,
                attributes: [.posixPermissions: 0o700])
            try JSONEncoder().encode(values).write(to: configURL, options: .atomic)
            try FileManager.default.setAttributes([.posixPermissions: 0o600], ofItemAtPath: configURL.path)
            actionNotice = "设备配置已保存"; render()
        } catch {
            let alert = NSAlert(); alert.messageText = "设备配置保存失败"; alert.runModal()
        }
    }
    ...
    private func indicator(_ color: NSColor) -> NSImage? {
        NSImage(systemSymbolName: "circle.fill", accessibilityDescription: "WeCom Capture")?.withSymbolConfiguration(.init(paletteColors: [color]))
    }
    ...
    private func writeDesktopHealth(_ access: DesktopAccess) {
        ...
        let directory = isStandalone ? supportURL.appendingPathComponent("logs") : URL(fileURLWithPath: "\(root)/.codex-run")
        do {
    ...
    private func controlAction(_ action: String, completion: (() -> Void)? = nil) {
        ...
        if isStandalone && ["start-edge", "restart-edge", "recover-history"].contains(action)
            && !FileManager.default.fileExists(atPath: configURL.path) {
            configureDevice(); return
        }
    ...
    private func ensureEdgePermissions() -> Bool {
        ...
        alert.informativeText = !access.accessibility
            ? "请在系统设置的隐私与安全性 > 辅助功能中授权 WeCom Capture。更新后如果已经开启，请移除旧条目，再添加 \(Bundle.main.bundlePath)。授权完成后重新点击启动边缘。"
            : "请在系统设置的隐私与安全性 > 屏幕与系统音频录制中允许 WeCom Capture，用于读取消息气泡方向和图片。授权后重新打开客户端，再点击启动边缘。"
    ...
    @objc private func openLogs() {
        NSWorkspace.shared.open(isStandalone ? supportURL.appendingPathComponent("logs") : URL(fileURLWithPath: "\(root)/.codex-run"))
    }
