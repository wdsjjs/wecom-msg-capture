...
**企业微信桌面消息采集工具，与可独立体验的消息运营工作台。**

桌面端通过 macOS 辅助功能（Accessibility）和屏幕识别读取会话、捕获图片，以 SQLite 维护消息状态，提供原生 Dock 应用和 Python CLI。浏览器演示台展示会话管理、接待切换、消息回复、设备状态和历史补录，可独立运行。

这是通用工具，不是企业微信官方 SDK。演示台使用虚构联系人、预设回复和模拟设备，不调用真实模型或发送真实消息。桌面端的持续同步与远程任务需要自建兼容的[通道服务](docs/channel-protocol.md)，浏览器演示不提供这个服务端。

![消息运营工作台，所有会话与设备均为虚构示例](docs/demo-preview.png)

## 体验工作台

[打开在线演示](https://wdsjjs.github.io/wecom-msg-capture/)

浏览器演示支持 Windows、macOS 和移动设备，不需要 Python、设备 Token 或企微账号。本地运行需要 Node.js 22.12+：
...
打开终端显示的本地地址。可搜索和切换会话、预览图片、生成预设草稿、人工回复、模拟新消息、切换 AI 接待、模拟设备离线，以及补录和恢复接收。页面始终标记为演示环境；刷新或重置会恢复初始数据。

```bash
npm test          # 状态逻辑回归
npm run build    # 输出可静态部署的 dist/
```

交互验证、数据边界与部署方式见[演示台说明](docs/demo.md)。
...
| 浏览器演示 | 虚构会话、接待开关、图片预览、回复模拟、设备状态及运行记录 |
...
| 路径 | 用途 |
| --- | --- |
| `demo/` | React + TypeScript 消息运营演示台，纯浏览器运行 |
