...
const context = await browser.newContext({
  viewport: { width: 1768, height: 950 },
});
...
try {
  ...
  await expect(
    page.getByRole("heading", { name: "企微消息工作台 6 个会话" }),
  ).toBeVisible();
  await expect(
    page.getByRole("img", { name: "虚构设备的连接状态示例" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("tab", { name: "未读 1", exact: true }),
  ).toBeVisible();
  await page.getByRole("tab", { name: "全部", exact: true }).click();
  await page.getByRole("button", { name: /01 访客 01/ }).click();

  await page
    .getByRole("button", { name: "查看图片：设备状态示例", exact: true })
    .click();
  await expect(page.getByRole("dialog")).toBeVisible();
  await expect(
    page.getByRole("img", { name: "虚构设备状态详情，无真实账号信息" }),
  ).toBeVisible();
  await close();
  ...
  await expect(
    page.getByRole("button", { name: "模拟收到新消息" }),
  ).toBeDisabled();
  ...
  await page
    .getByRole("switch", { name: "当前会话 AI 托管", exact: true })
    .click();
  ...
  await page.getByRole("button", { name: "历史补录", exact: true }).click();
  await expect(page.getByRole("dialog")).toContainText("3 个会话 · 7 条历史消息 · 0 条 AI 回复");
  await page
    .getByRole("button", { name: "结束恢复并继续接收", exact: true })
    .click();
  await close();
  assert.equal(await chat.locator(".outgoing").count(), during);
  await page.getByRole("button", { name: "模拟收到新消息" }).click();
  ...
  await page
    .getByRole("combobox", { name: "筛选运行记录" })
  ...
  await expect(
    page.getByRole("tab", { name: "未读 2", exact: true }),
  ).toBeVisible();
  ...
  for (const viewport of [
    ...
    if (viewport.width < 640) {
      ...
      await expect(
        chat.getByText("如果设备暂时离线，消息会丢失吗？", { exact: true }),
      ).toBeInViewport();
    }
    ...
    assert.ok(
      dimensions.width <= dimensions.client,
      JSON.stringify(dimensions),
    );
    assert.ok(
      dimensions.height <= dimensions.available,
      JSON.stringify(dimensions),
    );
  ...
  console.log(
    "PASS: search, unread, images, draft/send, offline, takeover, recovery, live resume, logs, reset, 4 responsive viewports, no external requests or browser errors.",
  );
} finally {
  await browser.close();
}
