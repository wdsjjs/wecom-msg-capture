...
describe('isolated demo behavior', () => {
  it('blocks sends while offline without queuing a later replay', () => {
    ...
    const sent = reducer(offline, { type: 'send', id, text: 'should not send' });
    expect(sent).toBe(offline);
    expect(reducer(sent, { type: 'toggleOnline' }).conversations).toEqual(original.conversations);
  });
  it('preserves live watermarks, unread counts and manual settings during recovery', () => {
    ...
    expect(reducer(ended, { type: 'beginRecovery' }).conversations).toEqual(recovering.conversations);
  });
  it('keeps observations during recovery history-only, then replies to truly new live input', () => {
    ...
    const observed = reducer(recovering, { type: 'receive', id, text: 'old spool message' });
    expect(observed.conversations[0].messages.at(-1)?.history).toBe(true);
    expect(latest(observed.conversations[0])).toEqual(latest(recovering.conversations[0]));
    expect(reducer(observed, { type: 'send', id, text: 'blocked' })).toBe(observed);
    ...
    expect(live.conversations[0].messages.slice(-2).map(m => m.direction)).toEqual(['inbound', 'outbound']);
    expect(live.conversations[0].messages.at(-1)?.author).toBe('ai');
  });
  it('honors global AI and conversation takeover independently', () => {
    for (const paused of [reducer(initialState(), { type: 'toggleGlobalAi' }), reducer(initialState(), { type: 'toggleAi', id })]) {
      const received = reducer(paused, { type: 'receive', id, text: '你好' });
      expect(received.conversations[0].messages).toHaveLength(paused.conversations[0].messages.length + 1);
      expect(received.conversations[0].messages.at(-1)?.direction).toBe('inbound');
    }
  });
  it('rejects whitespace and bounds user text', () => {
    const original = initialState();
    expect(reducer(original, { type: 'send', id, text: '  ' })).toBe(original);
    expect(reducer(original, { type: 'send', id, text: 'x'.repeat(2100) }).conversations[0].messages.at(-1)?.text).toHaveLength(2000);
  });
  it('resets all ephemeral state', () => {
    const changed = reducer(reducer(initialState(), { type: 'beginRecovery' }), { type: 'toggleOnline' });
